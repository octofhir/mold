...
#
# Downloads the latest stable release binary for this platform, verifies its
# SHA-256, and installs it. Override the destination with BANSHEE_INSTALL_DIR.
set -eu

REPO="octofhir/banshee"
BIN="banshee"
INSTALL_DIR="${BANSHEE_INSTALL_DIR:-$HOME/.local/bin}"

err() { echo "banshee-install: $*" >&2; exit 1; }
...
echo "banshee-install: downloading $archive"
...
echo "banshee-install: installed $("$INSTALL_DIR/$BIN" --version 2>/dev/null || echo "$BIN") to $INSTALL_DIR"
case ":$PATH:" in
  *":$INSTALL_DIR:"*) ;;
  *) echo "banshee-install: add $INSTALL_DIR to your PATH:  export PATH=\"$INSTALL_DIR:\$PATH\"" ;;
esac
