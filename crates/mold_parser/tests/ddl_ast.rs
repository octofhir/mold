...
fn drop_statement() {
    ...
    assert_eq!(drop.object_kind(), Some(mold_syntax::SyntaxKind::INDEX_KW));
...
fn statements_iter_yields_ddl_variants() {
    ...
    assert_eq!(
        kinds,
        vec![
            mold_syntax::SyntaxKind::CREATE_TABLE_STMT,
            mold_syntax::SyntaxKind::ALTER_STMT,
            mold_syntax::SyntaxKind::DROP_STMT,
        ]
    );
}
